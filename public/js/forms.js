...
const forms = {
	constructForm: function(formName) {
		...
		row.appendChild(
			constructButtonCol("Submit", "submit", [
				"btn-secondary",
				"form-submit",
			]),
		);
		...
		function constructFormSection(sectionDetail) {
			...
			let legendText = document.createTextNode(
				sectionDetail.sectionTitle,
			);
			legendEl.appendChild(legendText);

			for (let j = 0; j < sectionDetail.fields.length; j++) {
				fieldsetEl.appendChild(
					constructInputRow(sectionDetail.fields[j]),
				);
			}
			...
			function constructInputRow(fieldsArr) {
				...
				for (let k = 0; k < fieldsArr.length; k++) {
					...
					if (fieldDetail.readOnly)
						inputEl.setAttribute("readonly", true);

					if (fieldDetail.fieldType === "datalist")
						inputEl.setAttribute(
							"list",
							fieldDetail.fieldName + "List",
						);
	...
	setViewKeys: function() {
		...
		async function doWork(dim) {
			...
			try {
				...
				console.error(
					"Error in setViewKeys doWork async step for " +
						dim +
						" dimension",
				);
				console.error(err);
			...
			for (let i = 0; i < fieldLabels.length; i++) {
				...
				col.appendChild(
					document.createTextNode(
						lib.capitaliseWords(fieldLabels[i]) + ":",
					),
				);
				row.appendChild(col);
	...
	setJobDetails: function() {
		...
		for (let i = 0; i < jobDetail.length; i++) {
			let invoiceDateFormatted =
				typeof jobDetail[i].dateInvoiceSent === "string"
					? jobDetail[i].dateInvoiceSent
					: lib.googleDateNumberToGBFormat(
							jobDetail[i].dateInvoiceSent,
					  );
			...
			lib.appendOptionNode($jobDetailDatalist[0], concat).setAttribute(
				"data-key",
				jobDetail[i].jobId,
			);
		}
	...
	submitFormFlow: async function(form, formName, statusDiv) {
		...
		if (formAction === "new") {
			...
			function appendNewObj(dim, arr) {
				...
				obj[dim + "Id"] =
					document.appData[dim + "Detail"].slice(-1)[0][dim + "Id"] +
					1;
			...
			function updateObj(searchKey, updateDim, arr) {
				...
				let objArr = document.appData[updateDim + "Detail"].filter(
					val => {
	...
	validateClientForm: function() {
		...
		let $clientDetailsForm = $("#clientDetailsForm"),
			optionsNodeList = $("#jobDetailsForm")
				...
				.children(),
			accountName = $clientDetailsForm
				.find("#clientDetails-accountName")
				.val(),
			accountNameVals = [];
