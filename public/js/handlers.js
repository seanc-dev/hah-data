...
const handlers = {
		...
		$("#clientDetailsForm")
			...
			.on("input", function(e) {
				if (
					...
					if (value === undefined || value.search(reg) > -1)
						value = "";
	...
	handlerFormTypeSelect: function() {
		$(".form-type-select").change(function(e) {
			...
			toggleRecordSelectVisibility = toggleRecordSelectVisibility.bind(
				this,
			);
		...
		$(".form-record-select-div")
			...
			.change(function(e) {
				...
				axios
					...
					.then(result => {
						console.log(
							`handleRecordSelectChange handler axios result`,
						);
						console.log(result);
						...
						let $bodyEl =
							formAction === "Edit" ? $defaultBody : $viewBody;

						// loop through returned properties and apply as values to relevant cells
						for (let i = 0; i < keys.length; i++) {
							$bodyEl
								.find('[name="' + keys[i] + '"]')
								.val(result.data[keys[i]]);
							$contentEl
								.find(".status-message .alert")
								.alert("close");
						}

						if (dim === "job")
							lib.setJobAddressFields(
								$("#jobDetails-accountName").val(),
							);
					})
					.catch(err => {
						console.error(err);
