...
class DataObject {
    ...
    init(requestType) {
        ...
        let doWork = (function (cb, name) {
            ...
            console.log(`doWork name: ${name}, pr:`);
            console.log(pr);
        ...
        if (requestType === "view") return doWork(this.getDataFromSheet, "getDataFromSheet");
        if (requestType === "delete") return doWork(this.deleteDBObj, "deleteDBObj");
    ...
    async updateDBObj() {
        ...
        if (!this.data || !Object.keys(this.data).length || Object.keys(this.data).length < 1) {
