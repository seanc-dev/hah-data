...
class DataObject {
	...
	init(requestType) {
		let doWork = function(cb, name) {
			...
			// if (this.dimension === "job" && requestType !== "view") {
			// 	pr.then(result => {
			// 		setTimeout(this.initClientUpdate.bind(this), 0);
			// 	});
			// }
		...
		if (requestType === "new")
			return doWork(this.createDBObj, "createDBObj");
		if (requestType === "edit")
			return doWork(this.updateDBObj, "updateDBObj");
		if (requestType === "view")
	...
	async createDBObj() {
		...
		try {
			this.data[this.dimension + "Id"] = await this.generateId.bind(
				this,
			)();
		} catch (err) {
			console.error(err);
			console.error(
				"Error in this.generateId step in createDBObj for " +
					this.dimension +
					" instance with id " +
					this._id,
			);
		...
		try {
			...
			console.error(err);
			console.error(
				"Error in this.computeColumns step in createDBObj for " +
					this.dimension +
					" instance with id " +
					this._id,
			);
		...
		try {
			...
			console.error(err);
			console.error(
				"Error in this.appendRow step in createDBObj for " +
					this.dimension +
					" instance with id " +
					this._id,
			);
	...
	async updateDBObj() {
		...
		try {
			...
			console.error(
				"Error in updateSheetRow in " + this.dimension + " constructor",
			);
			console.error(err);
		}
	...
	async getDataFromSheet() {
		...
		try {
			...
			console.error(
				"Error in getDataFromSheet ss.getRowsArrayDataOnly return",
			);
			console.error(err);
		}
	...
	updateRowObject() {
		...
		if (!this.rowObject)
			return console.error(
				"DataObject instance has no rowObject property",
			);
		...
		for (let i = 0; i < this.mapping.length; i++) {
			...
			if (this.data[fieldName])
				this.rowObject[APIName] = this.data[fieldName];
		}
	}
