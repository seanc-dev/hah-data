...
class DataObject {
  ...
  async createDBObj(id) {
    // 1. append id to this.data dimension id
    // 2. compute columns
    // 3. append row to sheet and return id

    this.data[this.dimension + "Id"] = id;

    // compute dynamic column values
    let returnVal;
    try {
      // await this.computeColumns.bind(this)();
      await this.formatDbData.bind(this)();
    } catch (err) {
      console.error(err);
      console.error(
  ...
  async updateDBObj() {
    ...
    try {
      ...
      console.error(
        "Error in compute columns in " + this.dimension + " constructor"
      );
      console.error(err);
    }
  ...
  async formatDbData() {
    let data;
    try {
      data = await this.getDataFromDb();
    } catch (err) {
      console.error(err);
      throw err;
    }
    const keys = Object.keys(data);
    keys.forEach((dbHeader) => {
      this.data[
  ...
  async updateRowObject() {
    ...
    if (!this.rowObject) {
      try {
        ...
        console.error(err);
        return console.error(
          `updateRowObject function failed for ${this.organisationId} org, dim ${this.dimension}, id ${this._id} as rowObject not present on instance`
        );
      }
    }
