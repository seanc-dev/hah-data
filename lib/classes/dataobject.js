...
class DataObject {
	...
	async appendRow() {
		...
		for (let key in this.data) {
			data[getFieldLabelFromName(this.mapping, key)] = this.data[key];
		}
	...
	//   for (let key in this.data) {
	//     obj[getSheetsAPIName(this.mapping, key)] = this.data[key];
	//   }
	...
	async getDataFromSheet() {
		...
		for (let i = 0; i < this.mapping.length; i++) {
			...
			let fieldName = getFieldName(this.mapping, key);
