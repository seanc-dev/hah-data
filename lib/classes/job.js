...
class Job extends DataObject {
  ...
  async getDataFromDb() {
    const { getJobById } = require("../../routes/services/queries/job");
    try {
      this.data = await getJobById(this._id, this.organisationId);
    } catch (err) {
      throw err;
    }
  ...
  async computeColumns() {
    ...
    try {
      ...
      const staffRates = lib.getObjectWithMaxDateFromArray(
        result,
        "effectivedate"
      );
