...
class Client extends DataObject {
    ...
    async computeColumns() {
        ...
        try {
            rowsArr = await ss.getRowsArrayDataOnly(this.organisationId, "job", this.jobMapping, 1, "clientid = " + this._id);
        } catch (err) {
    ...
    accountNameFriendly() {
        ...
        if (this.data.accountName) this.data.accountNameFriendly = this.data.accountName.replace(/"'/g, "");
