...
module.exports = {
    ...
    addRow: function(organisationId, worksheetId, newRowObj){
        ...
        const doc = new GoogleSpreadsheet(config[process.env.ENVIRONMENT][organisationId]["targetSpreadsheetId"]);
    ...
    getRowsArrayDataOnly: function (organisationId, dimension, mapping, rowStart, query) {
        ...
        const   sheetIndex  = config[process.env.ENVIRONMENT][organisationId]["formTargets"][dimension].sheetIndex,
                doc         = new GoogleSpreadsheet(config[process.env.ENVIRONMENT][organisationId]["targetSpreadsheetId"]);
