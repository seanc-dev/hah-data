...
import validationFunctions from "./";
...
export default {
	...
	getNewFormObject: ({ clientDetails, jobDetails }) => {
		const getFormDetails = (detail) => {
			const { restfulName, structure, fields: fieldReference } = detail;
			return {
				restfulName,
				detail: structure.map(({ sectionTitle, fields }) => ({
					sectionTitle,
					rowsArray: fields.map((row) =>
						row.map((fieldName) => {
							const {
								label,
								fieldName: name,
							...
							return {
								...
								classes,
								validation: {
									isValidationRequired: !!validationDetail,
									validationDetail,
